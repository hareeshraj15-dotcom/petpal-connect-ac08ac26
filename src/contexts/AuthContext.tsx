...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signUp = async (email: string, password: string, name: string, selectedRole: AppRole) => {
    ...
    // If user was created and confirmed (auto-confirm is off, so they need to verify)
    if (data.user && data.user.identities && data.user.identities.length > 0) {
      // User created successfully, they will need to verify email
      return { error: null };
    }

    // If no identities, user already exists
    if (data.user && (!data.user.identities || data.user.identities.length === 0)) {
